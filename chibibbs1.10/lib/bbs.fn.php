...
*/
	...
	function news_pic($chibi_conn,$op){
		...
		$sql = "SELECT * FROM `chibi_pic` WHERE `type`='picture' AND ".$op." ORDER BY `idx` DESC LIMIT 0,5";
		}
		$query = mysql_query($sql,$chibi_conn);
		return $query;
	}
...
return $RetStr .= $ShortenStr; 
	...
	function news_comment($chibi_conn,$op){
		...
		$sql = "SELECT * FROM `chibi_comment` WHERE ".$op." ORDER BY `idx` DESC LIMIT 0,5";
		}
		$query = mysql_query($sql,$chibi_conn);
		return $query;
	}
	function pic($cid,$start,$end,$chibi_conn,$search,$keyword){
		if($search == "name")$string = "SELECT `chibi_pic`.* FROM  `chibi_pic` LEFT JOIN  `chibi_comment` ON  `chibi_pic`.`no` =  `chibi_comment`.`pic_no` WHERE  `chibi_comment`.`name`LIKE'".mysql_real_escape_string($keyword)."' AND  `chibi_comment`.`cid`='".mysql_real_escape_string($cid)."' GROUP BY `chibi_pic`.`no` ORDER BY  `chibi_pic`.`no` DESC LIMIT ".$start.",".$end;
		...
		else if($search == "no")$string = "SELECT * FROM `chibi_pic` WHERE cid='".mysql_real_escape_string($cid)."' AND no<='".mysql_real_escape_string($keyword)."' ORDER BY `no` DESC LIMIT ".$start.",".$end;
